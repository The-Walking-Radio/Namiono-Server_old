    ...
    public sealed class ShoutcastStream<T> : Member<T>
    {
            ...
            lock (listeners)
            {
                ...
                for (var i = 0; i < ls.Count; i++)
                {
                    ...
                    if (!listeners.ContainsKey((T)Convert.ChangeType(uid, typeof(T))))
                    {
                        ...
                        listener.UserAgent = ls.Item(i).SelectSingleNode("USERAGENT").InnerText;
        ...
        void Download()
        {
            ...
            using (var wc = new WebClient())
            {
                ...
                wc.DownloadStringAsync(new Uri(
                    string.Format("http://{0}:{1}/admin.cgi?mode=viewxml&pass={2}&sid={3}",
                    hostname, port, password, id)));
            }
        }
        ...
        public string MetaData
        {
            get
            {
                ...
                output += "\t<li><img src\"images/autodj.gif\" /></li>";
                ...
                lock (listeners)
                {
                    if (listeners.Count != 0)
                    {
                        foreach (var l in listeners.Values)
                        {
                            ...
                            var ts = new DateTime(new TimeSpan(ctime / 3600, ctime / 60, ctime / 60 / 60).Ticks).ToString("HH:mm");
                            ...
                            tmp += string.Format("<div class=\"tr sp_row\"><div class=\"td sc_col_id\"><h3>{0}</h3></div>" +
                                "<div class=\"td sc_col_ip\"><h3>{1}<br />({3}, {4})</h3></div><div class=\"td sc_col_ua\">{2}</div></div>",
                                  ts, l.IP, l.UserAgent, l.Country, l.City);
                        }
                    }
                    else
